...
namespace Util
{
  ...
  void String::append(unsigned length, const char *s)
  {
      _append(length, s);
  }
  ...
  const char & String::operator[] (unsigned i)
  const {
      assert(i < _length);
      return _text[i];
  } bool String::nextLine(FILE * fp)
  {
      assert(fp != NULL);
  ...
  void String::_append(unsigned length, const char *s)
  {
  ...
  void String::_append(const char *fmt, va_list ap)
  {
      int bufLen = 128;
      char *buf;
      ...
      while (1) {
          buf = new char[bufLen];
          int cnt = vsnprintf_s(buf, bufLen, _TRUNCATE, fmt, ap);
  ...
  void String::_grow(unsigned minSize)
  {
      if (minSize > _size) {
          char *old = _text;
          _size += minSize;
