...
namespace Group
{
           ...
           Util::Array2D <PointIC> &scanLines)
  {
      ...
      while (xi != x2 || yi != y2)
      {
        ...
        else
        {
          ...
          {
            intersected = max(boundaries.H(oldx,yi),intersected); 
            intersected = max(boundaries.H(xi,yi),intersected); 
            intersected = max(boundaries.V(xi,oldy),intersected); 
            intersected = max(boundaries.V(xi,yi),intersected); 
          }
          else if ((xi > oldx) && (yi < oldy)) //up to right
          {
            intersected = max(boundaries.H(oldx,oldy),intersected); 
            intersected = max(boundaries.H(xi,oldy),intersected); 
            intersected = max(boundaries.V(xi,oldy),intersected); 
            intersected = max(boundaries.V(xi,yi),intersected); 
          }
          else if ((xi < oldx) && (yi > oldy)) //down to left
          {
            intersected = max(boundaries.H(oldx,yi),intersected); 
            intersected = max(boundaries.H(xi,yi),intersected); 
            intersected = max(boundaries.V(oldx,oldy),intersected); 
            intersected = max(boundaries.V(oldx,yi),intersected); 
          }
                          ...
                          Util::Array1D<PointIC> &adj, int &count)
  {
      ...
      const int rxa = max(0,x0-wr);
      const int rya = max(0,y0-wr);
      const int rxb = min(x0+wr,width-1);
      const int ryb = min(y0+wr,height-1);
