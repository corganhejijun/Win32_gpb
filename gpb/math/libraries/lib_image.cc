...
namespace math {
    namespace libraries {
        ...
        matrix<> lib_image::grayscale(const matrix<>& r, const matrix<>& g, const matrix<>& b) {
            /* check arguments */
        ...
        void lib_image::rgb_gamma_correct(matrix<>& r, matrix<>& g, matrix<>& b, double gamma) {
            /* check arguments */
            matrix<>::assert_dims_equal(r._dims, g._dims);
            matrix<>::assert_dims_equal(r._dims, b._dims);
            /* gamma correct image */
        ...
        void lib_image::lab_normalize(matrix<>& l, matrix<>& a, matrix<>& b) {
            /* check arguments */
            ...
            for (unsigned long n = 0; n < l._size; n++) {
                ...
                if (l_val < 0) { l_val = 0; }
                else if (l_val > 1) { l_val = 1; }
                if (a_val < 0) { a_val = 0; }
                else if (a_val > 1) { a_val = 1; }
                if (b_val < 0) { b_val = 0; }
                else if (b_val > 1) { b_val = 1; }
                l._data[n] = l_val;
                a._data[n] = a_val;
                b._data[n] = b_val;
            }
         ...
         * interior data.
         */
        matrix<> lib_image::border_mirror_2D(const matrix<>& m, unsigned long border_x, unsigned long border_y) {
        ...
        cmatrix<> lib_image::border_mirror_2D(const cmatrix<>& m, unsigned long border_x, unsigned long border_y) {
            /* check that matrix is 2D */
            if (m._dims.size() != 2)
                throw ex_invalid_argument("matrix must be 2D");
            /* get matrix size */
         ...
         */
        matrix<> lib_image::border_trim_2D(const matrix<>& m, unsigned long border_x, unsigned long border_y) {
            /* check that matrix is 2D */
            if (m._dims.size() != 2)
                throw ex_invalid_argument("matrix must be 2D");
            /* get matrix size */
        ...
        cmatrix<> lib_image::border_trim_2D(const cmatrix<>& m, unsigned long border_x, unsigned long border_y) {
            /* check that matrix is 2D */
            if (m._dims.size() != 2)
                throw ex_invalid_argument("matrix must be 2D");
            /* get matrix size */
            ...
            unsigned long  overlap_x, unsigned long  overlap_y)
        {
            ...
            if (nx == 0) { nx = 1; }
            else if (nx > max_subdiv_x) { nx = max_subdiv_x; }
            if (ny == 0) { ny = 1; }
            else if (ny > max_subdiv_y) { ny = max_subdiv_y; }
         ...
         */
        matrix<> lib_image::resample_2D(const matrix<>& m, double scale_x, double scale_y) {
            /* check that matrix is 2D */
            if (m._dims.size() != 2)
                throw ex_invalid_argument("matrix must be 2D");
        ...
        cmatrix<> lib_image::resample_2D(const cmatrix<>& m, double scale_x, double scale_y) {
            /* check that matrix is 2D */
            if (m._dims.size() != 2)
                throw ex_invalid_argument("matrix must be 2D");
            /* compute desired size from scale factor */
        ...
        matrix<> lib_image::resample_2D(const matrix<>& m, unsigned long size_x, unsigned long size_y) {
            /* check that matrix is 2D */
            if (m._dims.size() != 2)
                throw ex_invalid_argument("matrix must be 2D");
            /* resample */
        ...
        cmatrix<> lib_image::resample_2D(const cmatrix<>& m, unsigned long size_x, unsigned long size_y) {
            /* check that matrix is 2D */
            if (m._dims.size() != 2)
                throw ex_invalid_argument("matrix must be 2D");
            /* resample */
        ...
        namespace {
            ...
            unsigned long support_x_rotated(unsigned long support_x, unsigned long support_y, double ori) {
            ...
            unsigned long support_y_rotated(unsigned long support_x, unsigned long support_y, double ori) {
                return static_cast<unsigned long>(
            ...
            unsigned long size_x_rotated(unsigned long size_x, unsigned long size_y, double ori) {
            ...
            unsigned long size_y_rotated(unsigned long size_x, unsigned long size_y, double ori) {
                double support_x = static_cast<double>(size_x) / 2;
                double support_y = static_cast<double>(size_y) / 2;
                return static_cast<unsigned long>(
        ...
        matrix<> lib_image::rotate_2D_crop(const matrix<>& m, double ori, unsigned long size_x, unsigned long size_y) {
            /* check that matrix is 2D */
            if (m._dims.size() != 2)
                throw ex_invalid_argument("matrix must be 2D");
            /* compute rotation */
        ...
        cmatrix<> lib_image::rotate_2D_crop(const cmatrix<>& m, double ori, unsigned long size_x, unsigned long size_y) {
            /* check that matrix is 2D */
            if (m._dims.size() != 2)
                throw ex_invalid_argument("matrix must be 2D");
            /* compute rotation */
         ...
         ***************************************************************************/
          ...
          */
        auto_collection< matrix<>, array_list< matrix<> > > lib_image::gaussian_pyramid(const matrix<>& m, double sigma) {
            /* check arguments */
            if (m._dims.size() != 2)
         ...
         */
        auto_collection< matrix<>, array_list< matrix<> > > lib_image::laplacian_pyramid(const matrix<>& m, double sigma) {
            auto_collection< matrix<>, array_list< matrix<> > > g_pyramid =
                lib_image::gaussian_pyramid(m, sigma);
            return lib_image::laplacian_pyramid(*g_pyramid);
        }

        auto_collection< matrix<>, array_list< matrix<> > > lib_image::laplacian_pyramid(const array_list< matrix<> >& g_pyramid) {
            /* allocate pyramid */
            auto_collection< matrix<>, array_list< matrix<> > > l_pyramid(new array_list< matrix<> >());
            /* compute pyramid */
        ...
        matrix<> lib_image::image_from_laplacian_pyramid(const array_list< matrix<> >& l_pyramid) {
            /* check that pyramid is nonempty */
         ...
         ***************************************************************************/
          ...
          */
        matrix<> lib_image::gaussian(double sigma, unsigned int deriv, bool hlbrt) {
            unsigned long support = static_cast<unsigned long>(math::ceil(3 * sigma));
            return lib_image::gaussian(sigma, deriv, hlbrt, support);
        }

        matrix<> lib_image::gaussian(double sigma, unsigned int deriv, bool hlbrt, unsigned long support) {
         ...
         * If returning a 1st or 2nd derivative, the kernel has zero mean.
         */
        matrix<> lib_image::gaussian_2D(double sigma_x, double sigma_y, double ori, unsigned int deriv, bool hlbrt) {
        ...
        matrix<> lib_image::gaussian_2D(
            ...
            unsigned long support_x, unsigned long support_y)
        {
         ...
         */
        matrix<> lib_image::gaussian_cs_2D(double sigma_x, double sigma_y, double ori, double scale_factor) {
            /* compute support from rotated corners of original support box */
            unsigned long support_x = static_cast<unsigned long>(math::ceil(3 * sigma_x));
            unsigned long support_y = static_cast<unsigned long>(math::ceil(3 * sigma_y));
            unsigned long support_x_rot = support_x_rotated(support_x, support_y, ori);
        ...
        matrix<> lib_image::gaussian_cs_2D(
            double sigma_x, double sigma_y, double ori,
            double scale_factor,
            unsigned long support_x, unsigned long support_y)
        {
         ...
         * Filters are created in parallel when possible.
         */
        auto_collection< matrix<>, array_list< matrix<> > > lib_image::oe_filters_even(unsigned long n_ori, double sigma) {
            return lib_image::gaussian_filters(n_ori, sigma, 2, false, 3.0);
        }

        auto_collection< matrix<>, array_list< matrix<> > > lib_image::oe_filters_odd(unsigned long n_ori, double sigma) {
            return lib_image::gaussian_filters(n_ori, sigma, 2, true, 3.0);
        }

        void lib_image::oe_filters( unsigned long n_ori, double sigma,
            ...
            auto_collection< matrix<>, array_list< matrix<> > >& filters_odd)
        {
            ...
            public:
                 ...
                 */
                virtual void run() {
                    _filters = _even_or_odd ? lib_image::oe_filters_odd(_n_ori, _sigma) : lib_image::oe_filters_even(_n_ori, _sigma);
                }
         ...
         * Filters are created in parallel when possible.
         */
        auto_collection< matrix<>, array_list< matrix<> > > lib_image::texton_filters(unsigned long n_ori, double sigma) {
            /* allocate collection to hold filters */
            auto_collection< matrix<>, array_list< matrix<> > > filters(
                new array_list< matrix<> >()
                );
            ...
            unsigned long support = static_cast<unsigned long>(math::ceil(3 * sigma));
            matrix<> f_cs = lib_image::gaussian_cs_2D(sigma, sigma, 0, M_SQRT2l, support, support);
         ...
         */
        auto_collection< matrix<>, array_list< matrix<> > > lib_image::filter(const matrix<>& m, const collection< matrix<> >& filters) {
            /* allocate collection to hold filter responses */
            auto_collection< matrix<>, array_list< matrix<> > > responses(new array_list< matrix<> >());
            /* allocate collection of filterers */
            auto_collection< runnable, list<runnable> > filterers(new list<runnable>());
            /* setup filterers */
            for (auto_ptr< iterator< matrix<> > > i = filters.iter_create(); i->has_next(); ) {
                matrix<>& f = i->next();
         ...
         * Filter responses are computed in parallel when possible.
         */
        auto_collection< matrix<>, array_list< matrix<> > > lib_image::filter_sq(const matrix<>& m, const collection< matrix<> >& filters) {
            /* allocate collection to hold filter responses */
        ...
        matrix< matrix<> > lib_image::vectorize_filter_responses(const collection< matrix<> >& filter_responses) {
         ...
         */
        matrix<unsigned long> lib_image::quantize_values(const matrix<>& m, unsigned long n_bins) {
            auto_collection< double, array_list<double> > centroids;
         ...
         */
        matrix<> lib_image::quantized_image(const matrix<unsigned long>& assignments, const array_list<double>& centroids) {
         ...
         ***************************************************************************/
          ...
          */
        void lib_image::edge_oe( const matrix<>& m, const matrix<>& fe, const matrix<>& fo,
            auto_ptr< matrix<> >& oe_even, auto_ptr< matrix<> >& oe_odd)
        {
            /* allocate result */
            ...
            auto_ptr< matrix<> >&         edge_ori)
        {
            ...
            array<double> oris = lib_image::standard_filter_orientations(n_ori);
            lib_image::combine_edge_oe(oes_even, oes_odd, oris, edge, edge_phase, edge_ori);
        }
            ...
            auto_ptr< matrix<> >&         edge_ori)
        {
            ...
            array<double> oris = lib_image::standard_filter_orientations(n_ori);
            lib_image::combine_edge_oe_rectified(oes_neg, oes_pos, oris, edge, edge_polarity, edge_ori);
        }
            ...
            auto_ptr< matrix<> >&         edge_ori)
        {
            ...
            if (oes_neg.size() != n_ori)
                throw ex_invalid_argument("number of oe- matrices different from number of orientations");
        ...
        matrix<unsigned long> lib_image::textons(
            ...
            double                                               subsampling)
        {
            return lib_image::textons(m, filters,
            ...
            auto_collection< matrix<>, array_list< matrix<> > >& textons)
        {
            ...
            auto_collection< matrix<>, array_list< matrix<> > > responses = lib_image::filter(m, filters);
        ...
        namespace {
            ...
            matrix<unsigned long> orientation_slice_map(unsigned long size_x, unsigned long size_y, unsigned long n_ori) {
                /* initialize map */
            ...
            void conv_in_place_1D(const matrix<>& m0, const matrix<>& m1, matrix<>& m) {
            ...
            const distanceable_functor<matrix<>, double>& f_dist)
        {
            ...
            return lib_image::hist_diff_2D(labels, region_mask, weights, smoothing_kernel, f_dist);
        }
            ...
            const distanceable_functor<matrix<>, double>& f_dist)
        {
            ...
            matrix<> weights = weight_matrix_disc(r_outer);
            /* compute histrogram difference */
            return lib_image::hist_diff_2D(histograms, region_mask, weights, smoothing_kernel, f_dist);
        }
            ...
            const distanceable_functor<matrix<>, double>& f_dist)
        {
            ...
            if ((w_size_x != r_size_x) || (w_size_y != r_size_y))
                throw ex_invalid_argument("dimensions of weight matrix must match dimensions of region mask");
            ...
            /* compute histograms and histogram differences at each location */
            compute_hist_gradient_2D(labels, weights, slice_map, vector(smoothing_kernel), slice_hist, gradients, f_dist);
            return h_diff;
        }
            ...
            const distanceable_functor<matrix<>, double>& f_dist)
        {
            ...
            matrix<unsigned long> slice_map(region_mask);
            /* compute histograms and histogram differences at each location */
            compute_hist_gradient_2D(histograms, weights, slice_map, vector(smoothing_kernel), slice_hist, gradients, f_dist);
            return h_diff;
        }
            ...
            const distanceable_functor<matrix<>, double>& f_dist)
        {
            ...
            return lib_image::hist_gradient_2D(labels, weights, n_ori, smoothing_kernel, f_dist);
        }
            ...
            const distanceable_functor<matrix<>, double>& f_dist)
        {
            ...
            matrix<> weights = weight_matrix_disc(r);
            /* compute oriented gradient histograms */
            return lib_image::hist_gradient_2D(histograms, weights, n_ori, smoothing_kernel, f_dist);
        }
            ...
            const distanceable_functor<matrix<>, double>& f_dist)
        {
            ...
            auto_collection< matrix<>, array_list< matrix<> > > gradients(new array_list< matrix<> >());
            ...
            matrix_resizer m_resizer1(*gradients, labels._dims, 0, n_ori - 1);
            m_resizer1.run();
            /* check that result is nonempty */
            if (labels._size == 0)
                return gradients;
            /* allocate matrices to hold histograms of each slice */
            auto_collection< matrix<>, array_list< matrix<> > > slice_hist(new array_list< matrix<> >());
            /* initialize slice histogram matrices */
            ...
            matrix_resizer m_resizer2(*slice_hist, slice_hist_dims, 0, 2 * n_ori - 1);
            m_resizer2.run();
            /* build orientation slice lookup map */
            matrix<unsigned long> slice_map = orientation_slice_map(w_size_x, w_size_y, n_ori);
            /* compute histograms and histogram differences at each location */
            compute_hist_gradient_2D(labels, weights, slice_map, vector(smoothing_kernel), *slice_hist, *gradients, f_dist);
            return gradients;
        }
            ...
            const distanceable_functor<matrix<>, double>& f_dist)
        {
            ...
            matrix_resizer m_resizer1(*gradients, histograms._dims, 0, n_ori - 1);
            m_resizer1.run();
            /* check that result is nonempty */
            if (histograms._size == 0)
                return gradients;
            ...
            auto_ptr< matrix<> >&         gradient_ori)
        {
            ...
            if (gradients.size() != n_ori)
                throw ex_invalid_argument("number of gradient matrices different from number of orientations");
         ...
         ***************************************************************************/
          ...
          */
        matrix<> lib_image::needleness(const matrix<>& f0, const matrix<>& f1, const matrix<>& f2, double epsilon) {
            /* check arguments */
            ...
            double                        epsilon)
        {
            ...
            auto_collection< matrix<>, array_list< matrix<> > > f_smoothed(new array_list< matrix<> >());
            for (unsigned long n = 0; n < n_ori; n++) {
                ...
                /* compute needleness */
                auto_ptr< matrix<> > f_result(new matrix<>(lib_image::needleness(f0, f1, f2, epsilon)));
        ...
        matrix<> lib_image::nonmax(const matrix<>& m, const matrix<bool>& mask) {
            ...
            if (!mask_valid)
                throw ex_invalid_argument("mask for d-dimensional matrix must be size 3^d");
            ...
            if (nmax._size > 0) {
                ...
                for (unsigned long n = 0; n < n_inds; n++) {
                    ...
                    bool is_max = true;
                    for (unsigned long n_off = 0; ((n_off < n_offsets) && (is_max)); n_off++)
                    {
         ...
         * NOTE: The original matrix must be nonnegative.
         */
        matrix<> lib_image::nonmax(const matrix<>& m, unsigned long d, bool allow_boundary) {
         ...
         * NOTE: The original matrix must be nonnegative.
         */
        matrix<> lib_image::nonmax_oriented_2D(const matrix<>& m, double ori, bool allow_boundary) {
        ...
        matrix<> lib_image::nonmax_oriented_2D(const matrix<>& m, const matrix<>& m_ori, double o_tol, bool allow_boundary) {
        ...
        namespace {
            ...
            void neighbor_indices_2D(
                ...
                unsigned long& ne, unsigned long& nw, unsigned long& se, unsigned long& sw)
            {
                neighbor_indices_2D(size_y, (x * size_y + y), n, s, e, w, ne, nw, se, sw);
            }
                ...
                T& n, T& s, T& e, T& w, T& ne, T& nw, T& se, T& sw)
            {
                ...
                neighbor_exists_2D(size_x, size_y, x, y, has_n, has_s, has_e, has_w, has_ne, has_nw, has_se, has_sw);
                ...
                neighbor_indices_2D(size_y, ind, ind_n, ind_s, ind_e, ind_w, ind_ne, ind_nw, ind_se, ind_sw);
            ...
            void neighbor_values_2D(
                ...
                T& n, T& s, T& e, T& w, T& ne, T& nw, T& se, T& sw)
            {
                neighbor_values_2D(m, size_x, size_y, x, y, (x * size_y + y), n, s, e, w, ne, nw, se, sw);
            }
                ...
                bool& ne, bool& nw, bool& se, bool& sw)
            {
                ...
                neighbor_values_2D(m, size_x, size_y, x, y, ind, v_n, v_s, v_e, v_w, v_ne, v_nw, v_se, v_sw);
                /* compare */
            ...
            void neighbor_values_compare_2D(
                ...
                bool& ne, bool& nw, bool& se, bool& sw)
            {
                neighbor_values_compare_2D(m, size_x, size_y, x, y, (x * size_y + y), v, n, s, e, w, ne, nw, se, sw);
            }
        } /* namespace */
        ...
        namespace {
             ...
             */
            bool is_thick_region_2D(bool n, bool s, bool e, bool w, bool ne, bool nw, bool se, bool sw) {
                return ((nw && n && w) || (n && ne && e) || (w && sw && s) || (e && s && se));
            }
             ...
             */
            bool is_erodable_2D(bool n, bool s, bool e, bool w, bool ne, bool nw, bool se, bool sw) {
                if (nw && n && w) {
                    return ((!e) && (!s) && (!se));
                }
                else if (n && ne && e) {
             ...
             * 3x3 neighborhood.
             */
            bool is_removable_2D(bool n, bool s, bool e, bool w, bool ne, bool nw, bool se, bool sw) {
                /* compute number of adjacent neighbors */
                unsigned long n_adjacent = (n ? 1 : 0) + (s ? 1 : 0) + (e ? 1 : 0) + (w ? 1 : 0);
                /* compute number of diagonal neighbors */
                unsigned long n_diagonal = (ne ? 1 : 0) + (nw ? 1 : 0) + (se ? 1 : 0) + (sw ? 1 : 0);
             ...
             */
            bool is_vertex_2D(bool n, bool s, bool e, bool w, bool ne, bool nw, bool se, bool sw) {
                /* compute number of adjacent neighbors */
        ...
        namespace skeleton_element_namespace{
            ...
            public:
                explicit skeleton_element(unsigned long x, unsigned long y, double v, bool is_erode, bool is_rem)
                    : x(x), y(y), value(v), is_erodable(is_erode), is_removable(is_rem) { }

                explicit skeleton_element(const skeleton_element& el)
                    : x(el.x), y(el.y), value(el.value), is_erodable(el.is_erodable), is_removable(el.is_removable) { }
             ...
             */
            class skeleton_element_priority_functor : public functors::comparable_functor<skeleton_element> {
                public:
                    int operator()(const skeleton_element& el0, const skeleton_element& el1) const {
                        static const compare_functor<bool>& f_cmp_bool = compare_functors<bool>::f_compare();
                        static const compare_functor<double>& f_cmp_double = compare_functors<double>::f_compare();
                        int cmp_quality = -(f_cmp_bool(el0.is_erodable, el1.is_erodable));
                        if (cmp_quality == 0)
                            cmp_quality = -(f_cmp_bool(el0.is_removable, el1.is_removable));
                        return ((cmp_quality == 0) ? f_cmp_double(el0.value, el1.value) : cmp_quality);
                    }
            };
             ...
             */
            class skeleton_element_compare_functor : public functors::comparable_functor<skeleton_element> {
                public:
                    int operator()(const skeleton_element& el0, const skeleton_element& el1) const {
                        static const compare_functor<unsigned long>& f_cmp = compare_functors<unsigned long>::f_compare();
                ...
                bool& is_removable)     /* is the element currently removable? */
            {
                ...
                neighbor_values_compare_2D(m, size_x, size_y, x, y, ind, 0.0, n, s, e, w, ne, nw, se, sw);
                n = !n;   s = !s;   e = !e;   w = !w; ne = !ne;  nw = !nw;  se = !se;  sw = !sw;
                /* check if element should be a candidate if it is not currently */
                is_candidate = is_candidate || is_thick_region_2D(n, s, e, w, ne, nw, se, sw);
                ...
                is_removable = (is_erodable) || (is_candidate && is_removable_2D(n, s, e, w, ne, nw, se, sw));
            }
        } /* namespace */
         ...
         ***************************************************************************/
          ...
          */
        matrix<unsigned long> lib_image::connected_components(const matrix<>& m) {
        ...
        matrix<unsigned long> lib_image::connected_components(const matrix<unsigned long>& m) {
            /* create default mask for full 3^d neighborhood */
            unsigned long n_dims = m._dims.size();
            array<unsigned long> mask_dims(n_dims, 3);
            matrix<bool> mask(mask_dims, true);
        ...
        matrix<unsigned long> lib_image::connected_components(const matrix<>& m, const matrix<bool>& mask) {
            /* create integer-valued matrix (map all nonzero values to 1) */
            matrix<unsigned long> m_nonzero(m._dims);
            for (unsigned long n = 0; n < m._size; n++) {
                if (m._data[n] != 0)
        ...
        matrix<unsigned long> lib_image::connected_components(const matrix<unsigned long>& m, const matrix<bool>& mask) {
            ...
            auto_collection< array<unsigned long>, list< array<unsigned long> > > q(new list< array<unsigned long> >());
            /* initialize labels */
            ...
            for (unsigned long n = 0; n < m._size; n++) {
                ...
                if ((val != 0) && (labels._data[n] == 0)) {
                    ...
                    {
                        auto_ptr< array<unsigned long> > pos_copy(new array<unsigned long>(pos));
                        q->add(*pos_copy);
                        pos_copy.release();
                    }
                    ...
                    while (!(q->is_empty())) {
                        ...
                        for (unsigned long d = 0; d < n_dims; d++) {
                            ...
                            unsigned long pos_end_m = ((p + s_right) < dims_m[d]) ? (p + s_right) : (dims_m[d] - 1);
                        ...
                        while (true) {
                            ...
                            if ((m._data[ind_m] == val) && (labels._data[ind_m] == 0) && (mask._data[ind_mask])) {
                                /* label */
         ...
         */
        matrix<unsigned long> lib_image::connected_components(const matrix<>& m, unsigned long d) {
            /* create integer-valued matrix (map all nonzero values to 1) */
            matrix<unsigned long> m_nonzero(m._dims);
            for (unsigned long n = 0; n < m._size; n++) {
                if (m._data[n] != 0)
        ...
        matrix<unsigned long> lib_image::connected_components(const matrix<unsigned long>& m, unsigned long d) {
            /* get # steps along dimension */
            unsigned long n_dims = m._dims.size();
            unsigned long n_steps = (d < n_dims) ? m._dims[d] : 1;
            /* initialize labels */
         ...
         * value in the matrix.
         */
        unsigned long lib_image::count_connected_components(const matrix<unsigned long>& labels) {
            return max(labels);
        }
        ...
        namespace {
                ...
                unsigned long size_y)         /* grid size in y-direction */
            {
                ...
                unsigned long n_steps = direction ? (y_end - y_start) : (y_start - y_end);
                ...
                unsigned long size_y)         /* grid size in y-direction */
            {
                ...
                unsigned long n_steps = direction ? (x_end - x_start) : (x_start - x_end);
                auto_ptr< array<unsigned long> > inds(new array<unsigned long>(n_steps + 1));
                unsigned long ind = x_start * size_y + y;
                (*inds)[0] = ind;
                for (unsigned long n = 1; n <= n_steps; n++) {
                    if (direction) { ind += size_y; }
                    else { ind -= size_y; }
                    (*inds)[n] = ind;
                }
                return inds;
                ...
                bool allow_diagonal = true)   /* allow diagonal connectivity? */
            {
                ...
                if (x_start_grid == x_end_grid) {
                    ...
                    return scan_vertical_line_segment(x_start_grid, y_start_grid, y_end_grid, size_y);
                }
                else if (y_start_grid == y_end_grid) {
                    /* horizontal line segment */
                    return scan_horizontal_line_segment(y_start_grid, x_start_grid, x_end_grid, size_y);
                }
                else {
                    ...
                    unsigned long max_steps_x = direction_x ? (x_end_grid - x_start_grid) : (x_start_grid - x_end_grid);
                    unsigned long max_steps_y = direction_y ? (y_end_grid - y_start_grid) : (y_start_grid - y_end_grid);
                    unsigned long max_steps = max_steps_x + max_steps_y;
                    auto_ptr< array<unsigned long> > inds(new array<unsigned long>(max_steps + 1));
                    /* initialize indices */
                ...
                unsigned long size_y)      /* grid size in y-direction */
            {
                ...
                auto_collection< array<unsigned long> > inds(new list< array<unsigned long> >());
                /* loop over boundary */
                r = math::abs(r);
                ...
                for (unsigned long n = 0; n < n_steps; n++, theta += step_size) {
                    ...
                    auto_ptr< array<unsigned long> > inds_segment = scan_line_segment(p, q, size_y, false);
                    /* store indices */
        ...
        namespace {
            ...
            auto_ptr<lib_image::contour_vertex> create_contour_vertex(unsigned long x, unsigned long y) {
            ...
            auto_ptr<lib_image::contour_edge> create_contour_edge(lib_image::contour_vertex& v_start, lib_image::contour_vertex& v_end) {
        ...
        point_2D lib_image::contour_edge::point(unsigned long n) const {
            return point_2D(static_cast<double>(this->x_coords[n]), static_cast<double>(this->y_coords[n]));
        }
            ...
            _edges_equiv(new array_list< list<contour_edge> >())
        {
            ...
            for (unsigned long ind = 0, x = 0; x < size_x; x++) {
                for (unsigned long y = 0; y < size_y; y++) {
                    ...
                    if (label != 0) {
                        ...
                        bool n = false, s = false, e = false, w = false;
                        bool ne = false, nw = false, se = false, sw = false;
                        neighbor_values_compare_2D(labels.data(), size_x, size_y, x, y, ind, label, n, s, e, w, ne, nw, se, sw);
                        /* check vertex conditions */
            ...
            for (unsigned long n = 0; n < n_labels; n++) {
                if (!has_vertex[n]) {
                    /* create vertex and set its id */
                    auto_ptr<contour_vertex> v = create_contour_vertex(label_x[n], label_y[n]);
                    v->id = _vertices->size();
                    /* record vertex assignment */
            ...
            for (unsigned long v_id = 0; v_id < _vertices->size(); v_id++) {
                ...
                for (unsigned long x = x_start; (x <= x_end) && (x < size_x); x += 2) {
                    for (unsigned long y = y_start; (y <= y_end) && (y < size_y); y += 2) {
                        if ((labels(x, y) == label) && (labels(v.x, y) != label) && (labels(x, v.y) != label)) {
                            q_x[n_neighbors] = x;
                            q_y[n_neighbors] = y;
                            n_neighbors++;
                        }
            ...
            _edges_equiv(new array_list< list<contour_edge> >())
        {
            ...
            for (unsigned long n = 0, n_e = contours._edges->size(); n < n_e; n++) {
                ...
                auto_ptr<contour_edge> e_copy = create_contour_edge((*_vertices)[e.vertex_start->id], (*_vertices)[e.vertex_end->id]);
                e_copy->id = e.id;
         ...
         ***************************************************************************/
          ...
          * add it to the set.
          */
        void lib_image::contour_set::trace_edge(const matrix<unsigned long>& labels, unsigned long v_id, unsigned long x, unsigned long y) {
            /* get matrix size */
            unsigned long size_x = labels.size(0);
            unsigned long size_y = labels.size(1);
            ...
            auto_collection< unsigned long, array_list<unsigned long> > e_x(new array_list<unsigned long>());
            auto_collection< unsigned long, array_list<unsigned long> > e_y(new array_list<unsigned long>());
            ...
            do {
                ...
                bool av_n = has_n ? ((labels[ind_n] == label) && (!_is_edge[ind_n])) : false;
                bool av_s = has_s ? ((labels[ind_s] == label) && (!_is_edge[ind_s])) : false;
                bool av_e = has_e ? ((labels[ind_e] == label) && (!_is_edge[ind_e])) : false;
                bool av_w = has_w ? ((labels[ind_w] == label) && (!_is_edge[ind_w])) : false;
                bool av_ne = has_ne ? ((labels[ind_ne] == label) && (!_is_edge[ind_ne])) : false;
                bool av_nw = has_nw ? ((labels[ind_nw] == label) && (!_is_edge[ind_nw])) : false;
                bool av_se = has_se ? ((labels[ind_se] == label) && (!_is_edge[ind_se])) : false;
                bool av_sw = has_sw ? ((labels[ind_sw] == label) && (!_is_edge[ind_sw])) : false;
            ...
            unsigned long n_edge_points = (endpoint_is_new_vertex ? (e_x->size() - 1) : (e_x->size()));
        ...
        void lib_image::contour_set::split_edge(unsigned long e_id, unsigned long p) {
            ...
            /* create split vertex */
            auto_ptr<contour_vertex> v_split = create_contour_vertex(e.x_coords[p], e.y_coords[p]);
        ...
        const lib_image::contour_vertex& lib_image::contour_set::vertex(unsigned long v_id) const {
        ...
        const lib_image::contour_edge& lib_image::contour_set::edge(unsigned long e_id) const {
            if (e_id >= _edges->size())
         ...
         */
        const list<lib_image::contour_edge>& lib_image::contour_set::edge_equiv(unsigned long equiv_id) const {
         ...
         * Throw an exception (ex_not_found) if no such vertex exists.
         */
        unsigned long lib_image::contour_set::vertex_id(unsigned long x, unsigned long y) const {
            if (!(_is_vertex(x, y)))
                throw ex_not_found("vertex not found at specified pixel location");
         ...
         * given pixel.  Throw an exception (ex_not_found) if no such edge exists.
         */
        unsigned long lib_image::contour_set::edge_id(unsigned long x, unsigned long y) const {
            if (!(_is_edge(x, y)))
                throw ex_not_found("edge not found at specified pixel location");
            return _assignments(x, y);
        }
        ...
        void lib_image::contour_set::subdivide_local(
            bool   enforce_angle, double max_angle, bool enforce_dist,
            double max_dist_dev, double tol_dist)
        {
            ...
            while (!(q.is_empty())) {
                ...
                for (unsigned long n = 0; n < n_points; n++) {
                    ...
                    if ((!split) && (enforce_angle)) {
                        ...
                        double angle_start = math::acos(dot(r_start_p, r_start_end) / (abs(r_start_p) * len_start_end));
                        double angle_end = math::acos(dot(r_end_p, -r_start_end) / (abs(r_end_p) * len_start_end));
        ...
        namespace {
             ...
             */
            unsigned long find_contour_point_closest_to_line(const lib_image::contour_edge& e, const point_2D& p, const point_2D& q) {
             ...
             * Throw an exception if the contour has no interior points.
             */
            unsigned long find_contour_point_closest_to_segment(const lib_image::contour_edge& e, const point_2D& p, const point_2D& q) {
                /* check that contour is nonempty */
                unsigned long n_points = e.size();
                if (n_points == 0)
                    throw ex_invalid_argument("contour has no interior points");
             ...
             * Throw an exception if the contour has no interior points.
             */
            unsigned long find_contour_point_farthest_from_line(const lib_image::contour_edge& e, const point_2D& p, const point_2D& q) {
                /* check that contour is nonempty */
                unsigned long n_points = e.size();
                if (n_points == 0)
                    throw ex_invalid_argument("contour has no interior points");
             ...
             * Throw an exception if the contour has no interior points.
             */
            unsigned long find_contour_point_farthest_from_segment(const lib_image::contour_edge& e, const point_2D& p, const point_2D& q) {
                /* check that contour is nonempty */
                unsigned long n_points = e.size();
                if (n_points == 0)
                    throw ex_invalid_argument("contour has no interior points");
        ...
        void lib_image::contour_set::subdivide_global() {
            ...
            for (unsigned long curr = 1; curr < n_edges; curr++) {
                ...
                if (e_compare(e_prev, e_curr) == 0) {
                    ...
                    if (can_split_prev && can_split_curr) {
                        ...
                        unsigned long pos_prev = find_contour_point_farthest_from_segment(e_prev, p, q);
                        unsigned long pos_curr = find_contour_point_farthest_from_segment(e_curr, p, q);
                        // compute distance from split point to line segment
                        double dist_prev = e_prev.point(pos_prev).distance_to_segment(p, q);
                        double dist_curr = e_curr.point(pos_curr).distance_to_segment(p, q);
                        ...
                        throw ex_invalid_argument("cannot subdivide indentical edges to be nonoverlapping");
                    }
                    // split edge
                    contour_edge* e = (split_prev) ? (&e_prev) : (&e_curr);
                    unsigned long split_pos = find_contour_point_farthest_from_segment(*e, p, q);
                    this->split_edge(e->id, split_pos);
            ...
            while (true) {
                ...
                auto_collection< point_2D, array_list<point_2D> > points(new array_list<point_2D>());
                unsigned long n_vertices = _vertices->size();
                for (unsigned long n = 0; n < n_vertices; n++) {
                    const contour_vertex& v = (*_vertices)[n];
                    auto_ptr<point_2D> p(new point_2D(static_cast<double>(v.x), static_cast<double>(v.y)));
                    points->add(*p);
                    p.release();
                }
                ...
                for (unsigned long n = 0; n < n_v; n++) {
                    ...
                    if (endpoint_ids.is_empty()) {
                        ...
                        for (unsigned long ne = 1; ne < n_interior; ne++) {
                            ...
                            unsigned long pos_a = find_contour_point_closest_to_segment(*ea, pb, qb);
                            double dist_a = ea->point(pos_a).distance_to_segment(pb, qb);
                            /* compute distance from contour eb to segment defined by ea */
                            unsigned long pos_b = find_contour_point_closest_to_segment(*eb, pa, qa);
                ...
                for (unsigned long n = 0; n < n_edges; n++) {
                    if (do_split[n]) {
                        ...
                        if (e.size() == 0) {
                            throw ex_invalid_argument("cannot subdivide edges to make topology consistent");
                        }
                        /* split edge */
            ...
            auto_ptr<triangulation>&                           cdt)
        {
            ...
            /* compute cdt */
            auto_ptr<triangulation> t = triangulation::delaunay(*points, constraint_start_ids, constraint_end_ids);
            ...
            for (unsigned long n = n_edges; n < n_edges_cdt; n++) {
                ...
                auto_ptr< array<unsigned long> > inds = scan_line_segment(p_start, p_end, size_y, true);
                unsigned long n_inds = inds->size();
        ...
        namespace {
             ...
             */
              ...
              */
            unsigned long contour_traversal_compute_offset(unsigned long x, unsigned long y, unsigned long x_next, unsigned long y_next) {
        ...
        namespace {
                ...
                unsigned long                    size_y)  /* grid size in y-direction */
            {
                ...
                auto_collection< array<unsigned long> > rays(new list< array<unsigned long> >());
                /* check that contour is nonempty */
                unsigned long n_points = e.size();
                if (n_points > 0) {
                    ...
                    point_2D p_ray_prev(width * math::cos(theta - delta), width * math::sin(theta - delta));
                    point_2D p_ray_next(width * math::cos(theta + delta), width * math::sin(theta + delta));
                    /* initialize traversal state */
                    int state = side ? contour_traversal_init_state_right(e) : contour_traversal_init_state_left(e);
                    /* traverse edge */
                    unsigned long ind = 0;
                    while ((ind < n_points) && (state != -1)) {
                        ...
                        if (is_in_grid(p, size_x, size_y)) {
                            ...
                            /* scan segments */
                            auto_ptr< array<unsigned long> > ray_prev = scan_line_segment(p, q_prev, size_y, false);
                            auto_ptr< array<unsigned long> > ray_next = scan_line_segment(p, q_next, size_y, false);
                            /* store scanned rays */
                ...
                unsigned long                       size_y)  /* grid size in y-direction */
            {
                ...
                unsigned long offset_prev = contour_traversal_compute_offset(v.x, v.y, x_prev, y_prev);
                unsigned long offset_next = contour_traversal_compute_offset(v.x, v.y, x_next, y_next);
                /* initialize traversal state along boundary of vertex */
                int state = side ? contour_traversal_init_state_right(offset_prev) : contour_traversal_init_state_left(offset_prev);
                ...
                while ((ind == 0) && (state != -1)) {
                    ...
                    if (is_in_grid(p, size_x, size_y)) {
                        /* scan disc */
                        auto_collection< array<unsigned long> > disc_rays = scan_disc(p, width, size_x, size_y);
                        rays->add(*disc_rays);
            ...
            public:
                ...
                virtual void run() {
                    ...
                    auto_collection< array<unsigned long> > rays_left = extract_edge_side(_e, false, _width, size_x, size_y);
                    auto_collection< array<unsigned long> > rays_right = extract_edge_side(_e, true, _width, size_x, size_y);
                    /* extract visible regions */
                    unsigned long id = _e.contour_equiv_id;
                    extract_visible_region(*rays_left, id, _id_map, _vis, _visibility, _region_left);
                    extract_visible_region(*rays_right, id, _id_map, _vis, _visibility, _region_right);
                }
            ...
            public:
                ...
                virtual void run() {
                    ...
                    unsigned long x_prev = use_e_prev ? _e_prev.x_coords[n_points_prev - 1] : v_prev->x;
                    unsigned long y_prev = use_e_prev ? _e_prev.y_coords[n_points_prev - 1] : v_prev->y;
                    /* compute coordinates of next point on contour */
                    unsigned long n_points_next = _e_next.size();
                    bool use_e_next = (n_points_next > 0);
                    unsigned long x_next = use_e_next ? _e_next.x_coords[0] : v_next->x;
                    unsigned long y_next = use_e_next ? _e_next.y_coords[0] : v_next->y;
            ...
            public:
                ...
                virtual void run() {
                    ...
                    for (list<lib_image::contour_edge>::iterator_t i(_equiv_set); i.has_next(); ) {
         ...
         */
        lib_image::region_set::region_set(const contour_set& contours, double width, double width_min) {
         ...
         */
        lib_image::region_set::region_set(const contour_set& contours, double width, double width_min, double scale) {
         ...
         */
        lib_image::region_set::region_set(const contour_set& contours, double width, double width_min,
            const array<double>& edge_vis, const matrix<>& visibility) {
                this->extract_regions(contours, width, width_min, edge_vis, visibility);
        }

        // Extract regions using a custom intervening contour cue.
        void lib_image::region_set::extract_regions(
            const contour_set& contours, double width, double width_min,
            const array<double>& edge_vis, const matrix<>& visibility)
        {
            ...
            auto_collection< runnable, list<runnable> > region_extractors(new list<runnable>());
            ...
            for (unsigned long equiv_id = 0; equiv_id < n_equiv; equiv_id++) {
                ...
                for (list<contour_edge>::iterator_t i(equiv_set); i.has_next(); ) {
                    ...
                    auto_ptr<edge_region_extractor> e_region_extractor(
                        new edge_region_extractor(e, w, id_map, edge_vis[e.id], visibility,
                        ...
                        auto_ptr<vertex_region_extractor> v_region_extractor(
                            new vertex_region_extractor(
                                *(e.vertex_start), *e_prev, e, w, id_map, vis, visibility,
                                _vertex_regions_left[e.vertex_start->id], _vertex_regions_right[e.vertex_start->id]));
            ...
            auto_collection< runnable, list<runnable> > region_combiners(new list<runnable>());
            /* setup region combiners */
            for (unsigned long equiv_id = 0; equiv_id < n_equiv; equiv_id++) {
                /* create combiner for left side of contour */
                auto_ptr<region_combiner> region_left_combiner(
                    new region_combiner(
                        (*(contours._edges_equiv))[equiv_id], _vertex_regions_left, _edge_regions_left,
                ...
                auto_ptr<region_combiner> region_right_combiner(
                    new region_combiner(
                        (*(contours._edges_equiv))[equiv_id], _vertex_regions_right, _edge_regions_right,
         ...
         */
        const array<unsigned long>& lib_image::region_set::vertex_region_left(unsigned long v_id) const {
         ...
         * Only subdivision vertices are associated with nonempty regions.
         */
        const array<unsigned long>& lib_image::region_set::vertex_region_right(unsigned long v_id) const {
        ...
        const array<unsigned long>& lib_image::region_set::edge_region_left(unsigned long e_id) const {
        ...
        const array<unsigned long>& lib_image::region_set::edge_region_right(unsigned long e_id) const {
         ...
         */
        const array<unsigned long>& lib_image::region_set::edge_equiv_region_left(unsigned long equiv_id) const {
         ...
         * edge equivalence class (original contour before subdivision).
         */
        const array<unsigned long>& lib_image::region_set::edge_equiv_region_right(unsigned long equiv_id) const {
            if (equiv_id >= _edge_equiv_regions_right.size())
                throw ex_index_out_of_bounds("invalid contour equivalence id", equiv_id);
            return _edge_equiv_regions_right[equiv_id];
        }

        matrix<> lib_image::line_inds(double x0, double y0, double x1, double y1, double sx) {
